...
int main() {
    ...
    // utf8_decode: ASCII
    {
        uint32_t cp; int n = utf8_decode("A", 0, cp);
        CHECK_EQ(n, 1);
        CHECK_EQ(cp, 0x41u);
    }
    // utf8_decode: 3-byte (中)
    {
        uint32_t cp; int n = utf8_decode("\xE4\xB8\xAD", 0, cp);
        CHECK_EQ(n, 3);
        CHECK_EQ(cp, 0x4E2Du);
    ...
    {
        std::string out;
        emit_wrapped(out, "the quick brown fox jumps over the lazy dog",
                     "", "", 15);
    ...
    {
        std::string out;
        emit_wrapped(out, "one two three four five six seven",
                     "* ", "  ", 12);
        CHECK(out.rfind("* ", 0) == 0);
    ...
    if (g_failed == 0) std::printf("test_wrap: OK\n");
