...
void scan_diff(std::string_view s, const TokenSink& emit) {
    ...
    while (i < n) {
        ...
        if (line.size() >= 3 &&
            (line.compare(0, 3, "+++") == 0 || line.compare(0, 3, "---") == 0)) {
...
void tokenise_diff(std::string_view s, const TokenSink& e) {
    scan_diff(s, e);
}
void tokenise_patch(std::string_view s, const TokenSink& e) {
    scan_diff(s, e);
}
...
void tokenise_gitconfig(std::string_view s, const TokenSink& emit) {
    ...
    while (i < n) {
        ...
        if (j < n && s[j] == '=') {
            size_t key_end = j;
            while (key_end > i && is_space(s[key_end - 1]))
                --key_end;
            emit(TokenKind::Attribute, s.substr(i, key_end - i));
            if (key_end < j)
                emit(TokenKind::Text, s.substr(key_end, j - key_end));
...
void tokenise_vimscript(std::string_view s, const TokenSink& emit) {
    static const char* const KW[] = {
        ...
        "endfor",    "function",  "endfunction", "return",   "let",      "unlet",    "set",
        "setlocal",  "setglobal", "map",         "nmap",     "vmap",     "imap",     "cmap",
        "noremap",   "nnoremap",  "vnoremap",    "inoremap", "cnoremap", "autocmd",  "augroup",
        "command",   "echo",      "echom",       "execute",  "call",     "try",      "catch",
        "endtry",    "finally",   "throw",       "source",   "runtime",  "syntax",   "colorscheme",
        "highlight", nullptr};
    size_t i = 0, n = s.size();
    while (i < n) {
        char c = s[i];
