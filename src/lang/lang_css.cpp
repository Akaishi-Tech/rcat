...
void scan(std::string_view s, const TokenSink& emit, bool scss, bool less) {
    ...
    while (i < n) {
        ...
        if (c == '{') { emit(TokenKind::Punctuation, s.substr(i, 1)); ++i; ++brace; in_value = false; continue; }
        if (c == '}') { emit(TokenKind::Punctuation, s.substr(i, 1)); ++i; --brace; in_value = false; continue; }
        if (c == ';') { emit(TokenKind::Punctuation, s.substr(i, 1)); ++i; in_value = false; continue; }
        if (c == ':') {
            emit(TokenKind::Operator, s.substr(i, 1));
            ++i;
            if (brace > 0) in_value = true;
            continue;
        }
        if (c == '"' || c == '\'') {
...
void tokenise_css (std::string_view s, const TokenSink& e) { scan(s, e, false, false); }
void tokenise_scss(std::string_view s, const TokenSink& e) { scan(s, e, true,  false); }
void tokenise_less(std::string_view s, const TokenSink& e) { scan(s, e, true,  true);  }
