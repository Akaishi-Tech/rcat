...
const char* const TY[] = {
    ...
    nullptr};
const char* const BI[] = {
    "abs","all","any","ascii","bin","bool","breakpoint","callable","chr",
    "classmethod","compile","delattr","dir","divmod","enumerate","eval","exec",
    "filter","format","getattr","globals","hasattr","hash","help","hex","id",
    "input","isinstance","issubclass","iter","len","locals","map","max","min",
    "next","oct","open","ord","pow","print","property","range","repr",
    "reversed","round","setattr","slice","sorted","staticmethod","sum","super",
...
void tokenise_python(std::string_view s, const TokenSink& emit) {
    ...
    while (i < n) {
        ...
        if (is_str_prefix_char(c)) {
            size_t p = i;
            int got = 0;
            while (p < n && is_str_prefix_char(s[p]) && got < 2) { ++p; ++got; }
            if (p < n && (s[p] == '"' || s[p] == '\'')) {
                char q = s[p];
                size_t start = i;
                // Triple-quoted?
