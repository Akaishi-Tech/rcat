...
void scan(std::string_view s, const TokenSink& emit, const Spec& sp) {
    ...
    while (i < n) {
        ...
        if (k < n && (s[k] == '=' || (sp.allow_colon_kv && s[k] == ':'))) {
            ...
            if (key_end < k)
                emit(TokenKind::Text, s.substr(key_end, k - key_end));
            // separator
            emit(TokenKind::Operator, s.substr(k, 1));
            i = k + 1;
            // optional space
            size_t sp_end = i;
            while (sp_end < n && (s[sp_end] == ' ' || s[sp_end] == '\t'))
                ++sp_end;
            if (sp_end > i)
                emit(TokenKind::Text, s.substr(i, sp_end - i));
            i = sp_end;
            ...
            if (i > v) {
                ...
                if (t == "true" || t == "false" || t == "null" || t == "True" || t == "False" ||
                    t == "yes" || t == "no") {
...
void tokenise_ini(std::string_view s, const TokenSink& e) {
    scan(s, e, INI);
}
void tokenise_env(std::string_view s, const TokenSink& e) {
    scan(s, e, ENV);
}
void tokenise_properties(std::string_view s, const TokenSink& e) {
    scan(s, e, PROPS);
}
