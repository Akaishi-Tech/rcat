...
const char* const kw_r[] = {"if",       "else",          "for",   "while", "repeat",
                            "function", "return",        "break", "next",  "in",
                            "TRUE",     "FALSE",         "NULL",  "NA",    "NA_integer_",
                            "NA_real_", "NA_character_", "Inf",   "NaN",   "library",
                            "require",  nullptr};
const char* const ty_r[] = {"numeric",    "integer", "double", "character", "logical",
                            "complex",    "raw",     "list",   "vector",    "matrix",
                            "data.frame", "factor",  "NULL",   nullptr};
const char* const bi_r[] = {
    "TRUE",   "FALSE",  "NULL",       "NA",           "Inf",        "NaN",    "c",
    "print",  "cat",    "paste",      "paste0",       "sprintf",    "length", "names",
    "attr",   "class",  "is.numeric", "is.character", "is.logical", "sum",    "mean",
    "median", "sd",     "var",        "min",          "max",        "sort",   "unique",
    "lapply", "sapply", "mapply",     "apply",        "do.call",    nullptr};
...
const char* const kw_jl[] = {
    ...
    "macro",    "module",     "mutable", "primitive", "quote",     "return", "struct",  "true",
    "try",      "type",       "using",   "while",     "where",     nullptr};
const char* const ty_jl[] = {
    "Int",     "Int8",    "Int16",         "Int32",          "Int64",   "Int128",  "UInt",
    "UInt8",   "UInt16",  "UInt32",        "UInt64",         "UInt128", "Float16", "Float32",
    "Float64", "Bool",    "Char",          "String",         "Symbol",  "Array",   "Vector",
    "Matrix",  "Dict",    "Set",           "Tuple",          "Nothing", "Any",     "Number",
    "Real",    "Integer", "AbstractFloat", "AbstractString", nullptr};
const char* const bi_jl[] = {"true",   "false", "nothing", "missing", "println", "print",
                             "push!",  "pop!",  "map",     "filter",  "reduce",  "collect",
                             "length", "size",  "typeof",  "isa",     "eltype",  nullptr};
...
void scan(std::string_view s, const TokenSink& emit, const Spec& sp) {
    ...
    while (i < n) {
        ...
        if (is_ident_start(c)) {
            ...
            if (in_keyword_set(word, sp.keywords))
                kk = TokenKind::Keyword;
            else if (in_keyword_set(word, sp.types))
                kk = TokenKind::BuiltinType;
            else if (in_keyword_set(word, sp.builtins))
...
void tokenise_r(std::string_view s, const TokenSink& e) {
    scan(s, e, {kw_r, ty_r, bi_r});
}
void tokenise_julia(std::string_view s, const TokenSink& e) {
    scan(s, e, {kw_jl, ty_jl, bi_jl});
}
