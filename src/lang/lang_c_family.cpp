...
const char* const kw_c[] = {
    ...
    "_Generic", "_Imaginary", "_Noreturn", "_Static_assert", "_Thread_local", nullptr};
const char* const ty_c[] = {
    "char",     "double",   "float",   "int",     "long",      "short",    "signed",
    "unsigned", "void",     "size_t",  "ssize_t", "ptrdiff_t", "intptr_t", "uintptr_t",
    "bool",     "int8_t",   "int16_t", "int32_t", "int64_t",   "uint8_t",  "uint16_t",
    "uint32_t", "uint64_t", "FILE",    "NULL",    nullptr};
const char* const bi_c[] = {"true", "false", "NULL", "stdin", "stdout", "stderr", "errno", nullptr};

const char* const kw_cpp[] = {"alignas",
                              "alignof",
                              "and",
                              "and_eq",
                              "asm",
...
const char* const ty_cpp[] = {
    ...
    "wchar_t",  "size_t",  "ssize_t",     "ptrdiff_t",  "intmax_t", "uintmax_t", "nullptr_t",
    "int8_t",   "int16_t", "int32_t",     "int64_t",    "uint8_t",  "uint16_t",  "uint32_t",
    "uint64_t", "string",  "string_view", "vector",     "map",      "set",       "array",
    "optional", "variant", "unique_ptr",  "shared_ptr", "weak_ptr", "function",  "span",
    nullptr};
const char* const bi_cpp[] = {"true", "false", "nullptr", "NULL", "this", "std", nullptr};

const char* const kw_java[] = {
    "abstract",  "assert",  "break",        "case",       "catch",      "class",   "const",
    "continue",  "default", "do",           "else",       "enum",       "extends", "final",
    "finally",   "for",     "goto",         "if",         "implements", "import",  "instanceof",
    "interface", "native",  "new",          "non-sealed", "package",    "permits", "private",
    "protected", "public",  "record",       "return",     "sealed",     "static",  "strictfp",
    "super",     "switch",  "synchronized", "this",       "throw",      "throws",  "transient",
    "try",       "var",     "volatile",     "while",      "yield",      nullptr};
const char* const ty_java[] = {
    "boolean", "byte",   "char",      "double", "float", "int",      "long",   "short", "void",
    "Boolean", "Byte",   "Character", "Double", "Float", "Integer",  "Long",   "Short", "String",
    "Object",  "Number", "List",      "Map",    "Set",   "Optional", "Stream", nullptr};
const char* const bi_java[] = {"true", "false", "null", "System", "Math", nullptr};
...
const char* const ty_js[] = {"Array",
                             ...
                             "Float64Array",
                             nullptr};
const char* const bi_js[] = {"true",     "false",   "null",    "undefined", "NaN",
                             "Infinity", "console", "window",  "document",  "globalThis",
                             "require",  "module",  "exports", "process",   nullptr};
...
const char* const kw_ts[] = {
    ...
    "type",       "typeof",  "unique",    "unknown",   "var",         "void",      "while",
    "with",       "yield",   nullptr};
const char* const ty_ts[] = {"any",     "boolean", "bigint",    "never",    "number",   "object",
                             "string",  "symbol",  "undefined", "unknown",  "void",     "Array",
                             "Promise", "Record",  "Partial",   "Readonly", "Required", "Pick",
                             "Omit",    "Map",     "Set",       nullptr};
// bi_js reused

const char* const kw_csharp[] = {
    "abstract",  "add",      "alias",    "as",         "ascending",  "async",     "await",
    "base",      "break",    "by",       "case",       "catch",      "checked",   "class",
    "const",     "continue", "default",  "delegate",   "descending", "do",        "dynamic",
    ...
    "internal",  "into",     "is",       "join",       "let",        "lock",      "nameof",
    "namespace", "new",      "not",      "null",       "object",     "on",        "operator",
    "orderby",   "out",      "override", "params",     "partial",    "private",   "protected",
    "public",    "readonly", "record",   "ref",        "remove",     "return",    "sealed",
    ...
    "where",     "while",    "with",     "yield",      nullptr};
const char* const ty_csharp[] = {
    ...
    "Object",     "SByte",       "Single", "String",  "UInt16", "UInt32", "UInt64", "List",
    "Dictionary", "IEnumerable", "Task",   nullptr};
const char* const bi_csharp[] = {"true", "false", "null", "this", "base", "Console", nullptr};

const char* const kw_go[] = {"break", "case",   "chan",        "const",     "continue", "default",
                             "defer", "else",   "fallthrough", "for",       "func",     "go",
                             "goto",  "if",     "import",      "interface", "map",      "package",
                             "range", "return", "select",      "struct",    "switch",   "type",
                             "var",   nullptr};
const char* const ty_go[] = {"bool",    "byte",    "complex64", "complex128", "error",  "float32",
                             "float64", "int",     "int8",      "int16",      "int32",  "int64",
                             "rune",    "string",  "uint",      "uint8",      "uint16", "uint32",
                             "uint64",  "uintptr", "any",       nullptr};
const char* const bi_go[] = {"true",    "false",   "nil",   "iota",   "append",  "cap",
                             "close",   "complex", "copy",  "delete", "imag",    "len",
                             "make",    "new",     "panic", "print",  "println", "real",
                             "recover", "min",     "max",   "clear",  nullptr};

const char* const kw_rust[] = {
    "as",    "async",  "await",  "break",  "const", "continue", "crate", "do",   "dyn",
    "else",  "enum",   "extern", "false",  "fn",    "for",      "if",    "impl", "in",
    "let",   "loop",   "match",  "mod",    "move",  "mut",      "pub",   "ref",  "return",
    "self",  "Self",   "static", "struct", "super", "trait",    "true",  "try",  "type",
    "union", "unsafe", "use",    "where",  "while", "yield",    nullptr};
const char* const ty_rust[] = {"bool",    "char",    "f32",      "f64",      "i8",     "i16",
                               "i32",     "i64",     "i128",     "isize",    "str",    "u8",
                               "u16",     "u32",     "u64",      "u128",     "usize",  "String",
                               "Vec",     "Box",     "Rc",       "Arc",      "Option", "Result",
                               "HashMap", "HashSet", "BTreeMap", "BTreeSet", nullptr};
const char* const bi_rust[] = {"true",          "false",       "Some",   "None",    "Ok",
                               "Err",           "Self",        "self",   "println", "print",
                               "eprintln",      "eprint",      "format", "vec",     "assert",
                               "assert_eq",     "assert_ne",   "dbg",    "panic",   "todo",
                               "unimplemented", "unreachable", "write",  "writeln", nullptr};
...
const char* const kw_kotlin[] = {
    ...
    "do",        "dynamic",   "else",       "enum",        "expect",   "external",    "false",
    ...
    "when",      "where",     "while",      "yield",       nullptr};
const char* const ty_kotlin[] = {"Any", "Array", "Boolean", "Byte",   "Char",   "Double", "Float",
                                 "Int", "Long",  "Nothing", "Short",  "String", "Unit",   "List",
                                 "Map", "Set",   "Pair",    "Triple", nullptr};
const char* const bi_kotlin[] = {
    "true",   "false", "null",  "this",          "super",        "println",      "print", "arrayOf",
    "listOf", "mapOf", "setOf", "mutableListOf", "mutableMapOf", "mutableSetOf", nullptr};

const char* const kw_scala[] = {
    "abstract", "case",     "catch",   "class",   "def",       "do",     "else",    "enum",
    "export",   "extends",  "false",   "final",   "finally",   "for",    "forSome", "given",
    "if",       "implicit", "import",  "lazy",    "macro",     "match",  "new",     "null",
    "object",   "override", "package", "private", "protected", "return", "sealed",  "super",
    "then",     "this",     "throw",   "trait",   "try",       "true",   "type",    "using",
    "val",      "var",      "while",   "with",    "yield",     nullptr};
const char* const ty_scala[] = {
    ...
    "Option", "Some",    "None",   "Either",  "Future", "Seq",    "Vector", nullptr};
const char* const bi_scala[] = {"true",  "false",   "null",  "this",
                                "super", "println", "print", nullptr};

const char* const kw_swift[] = {"actor",       "any",         "as",        "associatedtype",
                                "async",       "await",       "borrowing", "break",
                                "case",        "catch",       "class",     "consume",
                                "consuming",   "continue",    "default",   "defer",
                                "deinit",      "do",          "else",      "enum",
                                "extension",   "fallthrough", "false",     "fileprivate",
                                "for",         "func",        "guard",     "if",
                                "import",      "in",          "indirect",  "init",
                                "inout",       "internal",    "is",        "let",
                                "nonisolated", "open",        "operator",  "precedencegroup",
                                "private",     "protocol",    "public",    "repeat",
                                "rethrows",    "return",      "self",      "Self",
                                "static",      "struct",      "subscript", "super",
                                "switch",      "throw",       "throws",    "true",
                                "try",         "typealias",   "var",       "where",
                                "while",       nullptr};
const char* const ty_swift[] = {"Array",  "Bool",   "Character", "Dictionary", "Double", "Float",
                                "Int",    "Int8",   "Int16",     "Int32",      "Int64",  "Optional",
                                "Set",    "String", "UInt",      "UInt8",      "UInt16", "UInt32",
                                "UInt64", "Void",   "Any",       "AnyObject",  nullptr};
const char* const bi_swift[] = {"true",  "false", "nil",    "self",     "super",
                                "print", "Self",  "throws", "rethrows", nullptr};
...
const char* const ty_dart[] = {"bool",   "double", "dynamic", "int",    "num",      "String",
                               "List",   "Map",    "Set",     "Object", "Iterable", "Future",
                               "Stream", "Symbol", "void",    "Null",   nullptr};
const char* const bi_dart[] = {"true", "false", "null", "this", "super", "print", nullptr};

const char* const kw_objc[] = {"auto",
                               "break",
                               "case",
                               "const",
                               "continue",
                               ...
                               "IBAction",
                               nullptr};
const char* const ty_objc[] = {"char",       "double",       "float",    "int",      "long",
                               "short",      "signed",       "unsigned", "void",     "NSString",
                               "NSArray",    "NSDictionary", "NSSet",    "NSNumber", "NSInteger",
                               "NSUInteger", "CGFloat",      "NSObject", "NSError",  nullptr};
const char* const bi_objc[] = {"true", "false", "YES",  "NO",    "nil",
                               "Nil",  "NULL",  "self", "super", nullptr};
...
const char* const kw_php[] = {
    ...
    "endswitch", "endwhile",     "enum",       "extends",      "final",      "finally",
    "fn",        "for",          "foreach",    "function",     "global",     "goto",
    ...
    "xor",       "yield",        nullptr};
const char* const ty_php[] = {"array",    "bool",   "callable", "float",  "int",
                              "iterable", "mixed",  "never",    "object", "self",
                              "static",   "string", "void",     "parent", nullptr};
const char* const bi_php[] = {"true",      "false",  "null",     "this",    "echo",   "print",
                              "isset",     "empty",  "var_dump", "count",   "strlen", "is_array",
                              "is_string", "is_int", "is_bool",  "is_null", nullptr};
...
const char* const kw_groovy[] = {
    ...
    "implements", "import",    "in",      "instanceof",   "interface", "native", "new",
    "null",       "package",   "private", "protected",    "public",    "return", "static",
    "strictfp",   "super",     "switch",  "synchronized", "this",      "throw",  "throws",
    "trait",      "transient", "true",    "try",          "var",       "void",   "volatile",
    "while",      nullptr};
const char* const ty_groovy[] = {"boolean", "byte",  "char", "double", "float",  "int",
                                 "long",    "short", "void", "String", "Object", "List",
                                 "Map",     "Set",   "def",  nullptr};
const char* const bi_groovy[] = {"true",  "false",   "null",  "this",
                                 "super", "println", "print", nullptr};
...
const char* const kw_zig[] = {
    ...
    "usingnamespace", "var",         "volatile",  "while",    nullptr};
const char* const ty_zig[] = {"bool",     "void",         "noreturn",
                              "type",     "anytype",      "anyerror",
                              "anyframe", "comptime_int", "comptime_float",
                              "f16",      "f32",          "f64",
                              "f80",      "f128",         "i8",
                              "i16",      "i32",          "i64",
                              "i128",     "isize",        "u8",
                              ...
                              "c_void",   nullptr};
const char* const bi_zig[] = {"true", "false", "null", "undefined", "this", nullptr};

const char* const kw_v[] = {"as",       "asm",       "assert", "atomic",   "break",  "const",
                            "continue", "defer",     "else",   "enum",     "fn",     "for",
                            "go",       "goto",      "if",     "import",   "in",     "interface",
                            "is",       "isreftype", "lock",   "match",    "module", "mut",
                            "none",     "or",        "pub",    "return",   "rlock",  "select",
                            "shared",   "sizeof",    "spawn",  "static",   "struct", "type",
                            "typeof",   "union",     "unsafe", "volatile", nullptr};
const char* const ty_v[] = {"bool", "string", "i8",    "i16",     "int",     "i64",
                            "u8",   "u16",    "u32",   "u64",     "f32",     "f64",
                            "rune", "isize",  "usize", "voidptr", "byteptr", "charptr",
                            "any",  "map",    "array", nullptr};
const char* const bi_v[] = {"true",  "false",    "none", "println",
                            "print", "eprintln", "exit", nullptr};
...
const char* const ty_nim[] = {
    ...
    "uint64", "float",     "float32", "float64", "char",    "string", "bool",  "cstring", "seq",
    "array",  "openArray", "void",    "range",   "pointer", "object", nullptr};
const char* const bi_nim[] = {"true", "false", "nil", "echo", "result", "new",  "len",
                              "add",  "inc",   "dec", "high", "low",    nullptr};

const char* const kw_crystal[] = {
    "abstract",  "alias",    "annotation", "as",        "as?",
    "asm",       "begin",    "break",      "case",      "class",
    "def",       "do",       "else",       "elsif",     "end",
    "ensure",    "enum",     "extend",     "false",     "for",
    "fun",       "if",       "in",         "include",   "instance_sizeof",
    "is_a?",     "lib",      "macro",      "module",    "next",
    "nil",       "of",       "out",        "pointerof", "private",
    "protected", "require",  "rescue",     "return",    "select",
    "self",      "sizeof",   "struct",     "super",     "then",
    "true",      "type",     "typeof",     "union",     "unless",
    "until",     "verbatim", "when",       "while",     "with",
...
const char* const ty_crystal[] = {
    ...
    "Object", "Tuple",  "NamedTuple", "Range",   "Number", "Pointer", "Void",  nullptr};
const char* const bi_crystal[] = {"true", "false", "nil",   "self", "puts", "print",
                                  "p",    "pp",    "raise", "loop", nullptr};

const char* const kw_d[] = {
    "abstract",   "alias",     "align",     "asm",          "assert",    "auto",
    "body",       "break",     "case",      "cast",         "catch",     "class",
    "const",      "continue",  "debug",     "default",      "delegate",  "delete",
    "deprecated", "do",        "else",      "enum",         "export",    "extern",
    "false",      "final",     "finally",   "for",          "foreach",   "foreach_reverse",
    "function",   "goto",      "if",        "immutable",    "import",    "in",
    "inout",      "interface", "invariant", "is",           "lazy",      "macro",
    "mixin",      "module",    "new",       "nothrow",      "null",      "out",
    "override",   "package",   "pragma",    "private",      "protected", "public",
    "pure",       "ref",       "return",    "scope",        "shared",    "static",
    "struct",     "super",     "switch",    "synchronized", "template",  "this",
    "throw",      "true",      "try",       "typeid",       "typeof",    "union",
    "unittest",   "version",   "virtual",   "volatile",     "while",     "with",
    "__gshared",  "__traits",  "__vector",  "__parameters", nullptr};
const char* const ty_d[] = {"bool",   "byte",    "cdouble", "cent",   "cfloat",    "char",
                            "creal",  "dchar",   "double",  "float",  "idouble",   "ifloat",
                            "int",    "ireal",   "long",    "real",   "short",     "ubyte",
                            "ucent",  "uint",    "ulong",   "ushort", "void",      "wchar",
                            "string", "wstring", "dstring", "size_t", "ptrdiff_t", nullptr};
const char* const bi_d[] = {"true",    "false", "null",     "this",   "super",
                            "writeln", "write", "writefln", "writef", nullptr};
...
void scan(std::string_view s, const TokenSink& emit, const Dialect& d) {
    ...
    while (i < n) {
        ...
        // raw string prefix
        if (d.raw_string_prefix && (c == 'r' || c == 'R') && i + 1 < n &&
            (s[i + 1] == '"' || s[i + 1] == '#')) {
            size_t j = i + 1;
            int hashes = 0;
            while (j < n && s[j] == '#') {
        ...
        if (is_ident_start(c)) {
            ...
            if (in_keyword_set(word, d.keywords))
                kk = TokenKind::Keyword;
            else if (in_keyword_set(word, d.types))
                kk = TokenKind::BuiltinType;
            else if (in_keyword_set(word, d.builtins))
...
const Dialect D_Crystal = {kw_crystal, ty_crystal, bi_crystal, false, false,
                           false,      false,      false,      false};
const Dialect D_D = {kw_d, ty_d, bi_d, false, false, false, false, true, false};

}  // namespace

void tokenise_c(std::string_view s, const TokenSink& e) {
    scan(s, e, D_C);
}
void tokenise_cpp(std::string_view s, const TokenSink& e) {
    scan(s, e, D_Cpp);
}
void tokenise_java(std::string_view s, const TokenSink& e) {
    scan(s, e, D_Java);
}
void tokenise_javascript(std::string_view s, const TokenSink& e) {
    scan(s, e, D_Js);
}
void tokenise_typescript(std::string_view s, const TokenSink& e) {
    scan(s, e, D_Ts);
}
void tokenise_csharp(std::string_view s, const TokenSink& e) {
    scan(s, e, D_Cs);
}
void tokenise_go(std::string_view s, const TokenSink& e) {
    scan(s, e, D_Go);
}
void tokenise_rust(std::string_view s, const TokenSink& e) {
    scan(s, e, D_Rust);
}
void tokenise_kotlin(std::string_view s, const TokenSink& e) {
    scan(s, e, D_Kotlin);
}
void tokenise_scala(std::string_view s, const TokenSink& e) {
    scan(s, e, D_Scala);
}
void tokenise_swift(std::string_view s, const TokenSink& e) {
    scan(s, e, D_Swift);
}
void tokenise_dart(std::string_view s, const TokenSink& e) {
    scan(s, e, D_Dart);
}
void tokenise_objectivec(std::string_view s, const TokenSink& e) {
    scan(s, e, D_ObjC);
}
void tokenise_php(std::string_view s, const TokenSink& e) {
    scan(s, e, D_Php);
}
void tokenise_groovy(std::string_view s, const TokenSink& e) {
    scan(s, e, D_Groovy);
}
void tokenise_zig(std::string_view s, const TokenSink& e) {
    scan(s, e, D_Zig);
}
void tokenise_v(std::string_view s, const TokenSink& e) {
    scan(s, e, D_V);
}
void tokenise_nim(std::string_view s, const TokenSink& e) {
    scan(s, e, D_Nim);
}
void tokenise_crystal(std::string_view s, const TokenSink& e) {
    scan(s, e, D_Crystal);
}
void tokenise_d(std::string_view s, const TokenSink& e) {
    scan(s, e, D_D);
}
