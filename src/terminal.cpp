...
TerminalCaps detect_terminal_caps() {
    ...
    if (c.is_tty && c.color != ColorMode::None &&
        std::strcmp(term, "screen") != 0 &&
        std::strncmp(term, "tmux", 4) != 0) {
        if (env_set("VTE_VERSION")        ||
            env_set("KITTY_WINDOW_ID")    ||
            env_set("WT_SESSION")         ||
            env_set("KONSOLE_VERSION")    ||
            env_set("ALACRITTY_LOG")      ||
            env_set("WEZTERM_EXECUTABLE") ||
            env_set("GHOSTTY_RESOURCES_DIR") ||
            contains(term_program, "iTerm") ||
            contains(term_program, "WezTerm") ||
            contains(term_program, "Apple_Terminal") ||
            contains(term_program, "vscode") ||
            contains(term, "kitty")    ||
            contains(term, "foot")     ||
...
std::string sgr_reset() { return "\x1b[0m"; }
...
std::string sgr(const Style& s, ColorMode mode) {
    ...
    if (mode == ColorMode::TrueColor && s.use_fg_rgb) {
        ...
        out += ";38;2;" + std::to_string(r) + ";" +
                          std::to_string(g) + ";" +
                          std::to_string(b);
    } else if ((mode == ColorMode::Ansi256 || mode == ColorMode::TrueColor) &&
               s.fg_256 >= 0) {
