...
ImageRenderResult render_pixels(unsigned char* pixels, int w, int h, int max_width_cells,
                                int max_height_cells, ColorMode color) {
    ImageRenderResult r;
    if (max_width_cells < 1)
        max_width_cells = 1;
    ...
    chafa_canvas_draw_all_pixels(canvas, CHAFA_PIXEL_RGBA8_UNASSOCIATED, pixels, w, h, w * 4);
...
ImageRenderResult render_image_file(std::string_view path, int max_width_cells,
                                    int max_height_cells, ColorMode color) {
    ImageRenderResult r;
...
ImageRenderResult render_image_bytes(const unsigned char* bytes, size_t bytes_len,
                                     int max_width_cells, int max_height_cells, ColorMode color) {
    ImageRenderResult r;
    ...
    unsigned char* pixels =
        stbi_load_from_memory(bytes, static_cast<int>(bytes_len), &w, &h, &channels, 4);
    if (!pixels) {
        const char* reason = stbi_failure_reason();
...
WebFetchResult fetch_url(std::string_view url, int timeout_seconds, size_t max_bytes) {
    WebFetchResult r;
    std::string url_str(url);

    static bool inited = []() { return curl_global_init(CURL_GLOBAL_DEFAULT) == CURLE_OK; }();
    if (!inited) {
