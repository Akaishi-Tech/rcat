...
struct RS {
    ...
    std::string sgr_off() const {
        return color() ? sgr_reset() : std::string{};
    }

    Style top_style() const {
        return style_stack.empty() ? Style{} : style_stack.back();
    }

    void push_style(Style s) {
        style_stack.push_back(s);
        if (accumulating) inline_buf += sgr_open(s);
    }
    void pop_style() {
        if (!style_stack.empty()) style_stack.pop_back();
        if (accumulating) {
            inline_buf += sgr_off();
            if (!style_stack.empty()) inline_buf += sgr_open(style_stack.back());
        }
    }
    ...
    std::string block_prefix() const {
        ...
        if (!lists.empty()) p.append(lists.back().indent, ' ');
        return p;
    }

    int content_columns() const {
        return std::max(10, opts.caps.columns);
    }
};
...
bool looks_remote(std::string_view src) {
    static const std::string_view schemes[] = {
        "http://", "https://", "data:", "ftp://", "//"};
    for (auto s : schemes) {
        if (src.size() >= s.size() &&
            std::equal(s.begin(), s.end(), src.begin())) {
            return true;
        }
    }
    return false;
...
std::string resolve_local_path(std::string_view src, std::string_view doc_dir) {
    ...
    if (path_is_absolute(s) || doc_dir.empty()) return s;
...
void append_prefixed(std::string& out, std::string_view rendered,
...
void drain_pending_images(RS& st) {
    if (st.pending_images.empty()) return;
    ...
    for (auto& img : images) {
        ...
        if (!st.opts.no_images && !img.path.empty()) {
            ...
            if (img.remote) {
                if (st.opts.allow_web && web_fetch_available()) {
                    auto fetched = fetch_url(img.path,
                                             st.opts.web_timeout_seconds,
                                             st.opts.web_max_bytes);
                    if (fetched.success && !fetched.bytes.empty()) {
                        res = render_image_bytes(
                            reinterpret_cast<const unsigned char*>(
                                fetched.bytes.data()),
                            fetched.bytes.size(),
                            max_w, max_h, st.opts.caps.color);
                    }
                }
            } else {
                res = render_image_file(img.path, max_w, max_h,
                                        st.opts.caps.color);
            }
        ...
        if (!rendered) {
            ...
            line += "[";
            line += _("image: ");
            if (!img.alt.empty()) { line += img.alt; line += " — "; }
    ...
    if (st.lists.empty() && st.quote_depth == 0) st.out += "\n";
}
...
void emit_block(RS& st, std::string_view content) {
    ...
    if (!st.lists.empty()) {
        ...
        std::string style_open = st.color() && !st.style_stack.empty()
                                  ? st.sgr_open(st.top_style())
                                  : std::string{};
...
void render_table(RS& st) {
    ...
    size_t cols = 0;
    for (auto& r : t.rows) cols = std::max(cols, r.cells.size());
    if (cols == 0) return;
    while (t.aligns.size() < cols) t.aligns.push_back(MD_ALIGN_DEFAULT);
    ...
    for (size_t ri = 0; ri < t.rows.size(); ++ri) {
        ...
        for (size_t i = 0; i < cols; ++i) {
            line += ' ';
            std::string_view cell = i < r.cells.size()
                                       ? std::string_view(r.cells[i])
                                       : std::string_view{};
...
int enter_block_cb(MD_BLOCKTYPE type, void* detail, void* userdata) {
    ...
    switch (type) {
        ...
        case MD_BLOCK_LI: {
            ...
            if (!st.lists.empty()) {
                ...
                if (d && d->is_task) {
                    ...
                    lc.pending = st.opts.caps.unicode
        ...
        case MD_BLOCK_H: {
            ...
            st.push_style(s);
            for (unsigned i = 0; i < d->level; ++i) st.inline_buf += '#';
            st.inline_buf += ' ';
            break;
        }
        ...
        case MD_BLOCK_HTML: {
            st.inline_buf.clear();
            st.accumulating = true;
            Style s; s.dim = true; st.push_style(s);
            break;
        }
        ...
        case MD_BLOCK_TD: {
            ...
            if (d && st.table.aligns.size() <
                ...
                if (st.table.aligns.size() < want) st.table.aligns.push_back(d->align);
            }
            st.inline_buf.clear();
            st.accumulating = true;
            if (type == MD_BLOCK_TH) {
                Style s; s.bold = true; st.push_style(s);
            }
            break;
        }
...
int leave_block_cb(MD_BLOCKTYPE type, void* /*detail*/, void* userdata) {
    ...
    switch (type) {
        ...
        case MD_BLOCK_QUOTE:
            st.quote_depth--;
            if (st.quote_depth == 0 && st.lists.empty()) st.out += "\n";
            break;
        ...
        case MD_BLOCK_TH:
        case MD_BLOCK_TD: {
            if (type == MD_BLOCK_TH) st.pop_style();
...
int enter_span_cb(MD_SPANTYPE type, void* detail, void* userdata) {
    ...
    switch (type) {
        ...
        case MD_SPAN_A: {
            ...
            std::string url;
            if (d && d->href.text) url.assign(d->href.text, d->href.size);
        ...
        case MD_SPAN_IMG: {
            ...
            if (d && d->src.text) st.pending_image_url.assign(d->src.text, d->src.size);
            Style s = st.top_style();
...
int leave_span_cb(MD_SPANTYPE type, void* /*detail*/, void* userdata) {
    ...
    switch (type) {
        ...
        case MD_SPAN_A: {
            ...
            if (!st.link_urls.empty()) st.link_urls.pop_back();
            st.pop_style();
            if (st.opts.caps.hyperlinks && !st.opts.no_hyperlinks && !url.empty()) {
                ...
                if (!st.style_stack.empty()) st.inline_buf += st.sgr_open(st.style_stack.back());
                st.inline_buf += ")";
            }
            break;
        }
        case MD_SPAN_IMG: {
            ...
            if (can_render) {
                RS::PendingImage pi;
                pi.path   = remote ? src
                                   : resolve_local_path(src, st.opts.doc_dir);
                pi.remote = remote;
...
int text_cb(MD_TEXTTYPE type, const MD_CHAR* text, MD_SIZE size, void* userdata) {
    ...
    if (st.in_inline_code) {
        if (type == MD_TEXT_BR)        st.inline_code_buf += '\n';
        else if (type == MD_TEXT_SOFTBR) st.inline_code_buf += ' ';
        else                            st.inline_code_buf.append(text, size);
        return 0;
    }
    switch (type) {
        case MD_TEXT_BR:
...
bool render_markdown(std::string_view source,
