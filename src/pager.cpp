...
bool icontains(std::string_view hay, std::string_view needle) {
    ...
    for (std::size_t i = 0; i <= last; ++i) {
        std::size_t k = 0;
        for (; k < needle.size(); ++k) {
            if (ascii_lower(hay[i + k]) != ascii_lower(needle[k])) break;
        }
...
bool line_matches(std::string_view line, std::string_view needle,
                  bool case_sensitive) {
...
find_next_match(const std::vector<std::string_view>& lines,
    ...
    for (std::size_t i = from; i < lines.size(); ++i) {
        if (line_matches(lines[i], needle, case_sensitive)) return i;
    }
    return std::nullopt;
}
...
std::size_t clamp_top(std::ptrdiff_t top, std::size_t viewport,
                      std::size_t total) {
    if (total == 0) return 0;
    std::ptrdiff_t max_top = static_cast<std::ptrdiff_t>(total) -
                             static_cast<std::ptrdiff_t>(viewport);
    if (max_top < 0) max_top = 0;
    if (top < 0) return 0;
    if (top > max_top) return static_cast<std::size_t>(max_top);
...
void on_sigwinch(int) { g_resized.store(true, std::memory_order_relaxed); }
...
public:
    ...
    bool setup() {
        ...
        if (!isatty(fd_)) { ::close(fd_); fd_ = -1; return false; }

        if (tcgetattr(fd_, &old_) != 0) { ::close(fd_); fd_ = -1; return false; }
        have_old_ = true;
        ...
        raw.c_cc[VMIN]  = 1;
        raw.c_cc[VTIME] = 0;
        if (tcsetattr(fd_, TCSAFLUSH, &raw) != 0) return false;
...
int read_byte(int fd) {
    ...
    for (;;) {
        ...
        if (n == 0) return -1;
        if (errno == EINTR) {
            if (g_resized.load(std::memory_order_relaxed)) return -2; // resize signal
            continue;
        }
        return -1;
    }
...
enum Key {
    ...
    K_ENTER, K_BACKSPACE, K_ESC,
};

// Read one logical key (a byte or a parsed escape sequence).
int read_key(int fd) {
    int b = read_byte(fd);
    if (b == -2) return K_RESIZE;
    if (b < 0)   return K_EOF;

    if (b == '\r' || b == '\n') return K_ENTER;
    if (b == 0x7F || b == 0x08) return K_BACKSPACE;
    if (b != 0x1b) return b;
    ...
    if (b1 == '[' || b1 == 'O') {
        ...
        switch (b2) {
            ...
            case 'F': return K_END;
            case '5': case '6': case '1': case '4': case '7': case '8': {
                // CSI <digits> ~  — keypad-style. Drain to '~' or final.
                int term = 0;
                while ((term = read_byte_timeout(fd, 50)) >= 0) {
                    if (term == '~' || (term >= 0x40 && term <= 0x7E)) break;
                }
                if (b2 == '5') return K_PGUP;
                if (b2 == '6') return K_PGDN;
                if (b2 == '1' || b2 == '7') return K_HOME;
                if (b2 == '4' || b2 == '8') return K_END;
                return K_NONE;
            }
...
std::string status_text(const PagerOptions& opts,
                        std::size_t top, std::size_t viewport,
                        std::size_t total,
                        std::string_view message) {
    const std::string name = opts.filename.empty() ? std::string("(stdin)")
                                                   : opts.filename;
    std::size_t shown_end = std::min(total, top + viewport);
    int pct = total == 0 ? 100
                         : static_cast<int>((shown_end * 100) / total);

    char buf[256];
    if (total == 0) {
        std::snprintf(buf, sizeof(buf), "%s  (empty)", name.c_str());
    } else if (shown_end >= total) {
        std::snprintf(buf, sizeof(buf), "%s  lines %zu-%zu/%zu  END",
                      name.c_str(),
                      top + 1, shown_end, total);
    } else {
        std::snprintf(buf, sizeof(buf), "%s  lines %zu-%zu/%zu  %d%%",
                      name.c_str(),
                      top + 1, shown_end, total, pct);
    }
    std::string out = buf;
    if (!message.empty()) {
        out += "   ";
...
std::string truncate_to_width(std::string_view line, int max_cells) {
    if (max_cells <= 0) return std::string{};
    std::string out;
    ...
    for (std::size_t i = 0; i < line.size();) {
        ...
        if (width + w > max_cells) { truncated = true; break; }
...
void paint_frame(TtyGuard& tty,
    ...
    // Status bar.
    frame += "\x1b[2K";
    if (!opts.no_color) frame += "\x1b[7m";
    std::string status = status_text(opts, top,
...
std::optional<std::string> prompt(TtyGuard& tty, int rows, int cols,
    ...
    for (;;) {
        ...
        if (k == K_RESIZE)    { repaint(); continue; }
        if (k == K_ENTER)     return buf;
        if (k == K_ESC)       return std::nullopt;
        if (k == K_BACKSPACE) { if (!buf.empty()) buf.pop_back(); repaint(); continue; }
        if (k >= 0x20 && k < 0x7F) { buf.push_back(static_cast<char>(k)); repaint(); }
    }
}
...
int run_pager(std::string rendered, const PagerOptions& opts) {
    ...
    if (opts.rerender) rendered = opts.rerender(sz.cols);
    ...
    auto refresh_layout = [&] {
        ...
        top = clamp_top(static_cast<std::ptrdiff_t>(top),
                        static_cast<std::size_t>(std::max(1, sz.rows - 1)),
                        lines.size());
    };
    ...
    for (;;) {
        ...
        if (k == K_RESIZE) { refresh_layout(); continue; }
        if (k == K_EOF)    return 0;
        ...
        switch (k) {
            case 'q': case 'Q': case 0x03: // Ctrl-C
                return 0;
            ...
            case 'k': case K_UP:
                move_to(static_cast<std::ptrdiff_t>(top) - 1);
                break;
            case ' ': case 'f': case K_PGDN:
                move_to(static_cast<std::ptrdiff_t>(top) +
                        static_cast<std::ptrdiff_t>(viewport));
                break;
            case 'b': case K_PGUP:
                move_to(static_cast<std::ptrdiff_t>(top) -
            ...
            case 'd':
                move_to(static_cast<std::ptrdiff_t>(top) +
                        static_cast<std::ptrdiff_t>(viewport / 2));
                break;
            case 'u':
                move_to(static_cast<std::ptrdiff_t>(top) -
                        static_cast<std::ptrdiff_t>(viewport / 2));
            ...
            case '?': {
                ...
                if (!q || q->empty()) { message = _("Search cancelled"); break; }
                last_query = *q;
                last_query_forward = forward;
                auto m = forward
                    ? find_next_match(lines, last_query, top, false)
                    : find_prev_match(lines, last_query,
                                      top == 0 ? 0 : top - 1, false);
                if (m) {
                    top = clamp_top(static_cast<std::ptrdiff_t>(*m),
                                    viewport, lines.size());
                    char buf[128];
            ...
            case 'n': {
                if (last_query.empty()) { message = _("No previous search"); break; }
                std::size_t start = last_query_forward ? top + 1 : top;
                auto m = last_query_forward
                    ? find_next_match(lines, last_query, start, false)
                    : find_prev_match(lines, last_query,
                                      start == 0 ? 0 : start - 1, false);
                if (m) top = clamp_top(static_cast<std::ptrdiff_t>(*m),
                                       viewport, lines.size());
                else   message = _("Pattern not found");
                break;
            }
            case 'N': {
                if (last_query.empty()) { message = _("No previous search"); break; }
                std::size_t start = last_query_forward ?
                    (top == 0 ? 0 : top - 1) : top + 1;
                auto m = last_query_forward
                    ? find_prev_match(lines, last_query, start, false)
                    : find_next_match(lines, last_query, start, false);
                if (m) top = clamp_top(static_cast<std::ptrdiff_t>(*m),
                                       viewport, lines.size());
                else   message = _("Pattern not found");
