...
DECL(c);          DECL(cpp);        DECL(java);       DECL(javascript);
DECL(typescript); DECL(csharp);     DECL(go);         DECL(rust);
DECL(kotlin);     DECL(scala);      DECL(swift);      DECL(dart);
DECL(objectivec); DECL(php);        DECL(groovy);     DECL(zig);
DECL(v);          DECL(nim);        DECL(crystal);    DECL(d);

DECL(python);     DECL(ruby);       DECL(perl);       DECL(r);
DECL(julia);      DECL(shell);      DECL(powershell); DECL(tcl);

DECL(sql);        DECL(lua);        DECL(haskell);    DECL(elm);
DECL(ada);

DECL(lisp);       DECL(scheme);     DECL(clojure);    DECL(asm);

DECL(ocaml);      DECL(fsharp);
DECL(elixir);     DECL(erlang);

DECL(html);       DECL(xml);        DECL(svg);
DECL(css);        DECL(scss);       DECL(less);

DECL(json);       DECL(json5);      DECL(yaml);       DECL(toml);
DECL(ini);        DECL(env);        DECL(properties);

DECL(makefile);   DECL(dockerfile); DECL(cmake);      DECL(nginx);
DECL(apache);

DECL(diff);       DECL(patch);      DECL(gitconfig);  DECL(gitignore);
DECL(vimscript);  DECL(markdown);   DECL(latex);
...
std::string_view extension(std::string_view name) {
    ...
    if (pos == std::string_view::npos || pos + 1 == name.size()) return {};
...
Language lookup(std::string_view key, const Alias* table, size_t count) {
    if (key.empty()) return Language::PlainText;
    std::string k = ascii_lower(key);
    for (size_t i = 0; i < count; ++i) {
        if (k == table[i].key) return table[i].lang;
    }
...
Language from_shebang(std::string_view line) {
    ...
    auto tab = bn.find('\t');
    if (tab != std::string_view::npos) bn = bn.substr(0, tab);
    ...
    if (bn.empty() || bn == "env") return Language::PlainText;
...
Language detect_language(std::string_view info,
    ...
    if (!info.empty()) {
        ...
        if (l != Language::PlainText) return l;
    }
    if (!filename.empty()) {
        auto bn = basename(filename);
        Language l = lookup(bn, kFilenameMap,
                            sizeof(kFilenameMap) / sizeof(*kFilenameMap));
        if (l != Language::PlainText) return l;
...
Style style_for(TokenKind kind) {
    Style s;
    switch (kind) {
        case TokenKind::Text:                            break;
        case TokenKind::Comment:      s.fg_256 = 245; s.italic = true; break;
        case TokenKind::Keyword:      s.fg_256 = 135; s.bold = true;   break;
        case TokenKind::BuiltinType:  s.fg_256 = 81;                    break;
        case TokenKind::Builtin:      s.fg_256 = 173;                   break;
        case TokenKind::Function:     s.fg_256 = 220;                   break;
        case TokenKind::Constant:     s.fg_256 = 209;                   break;
        case TokenKind::Number:       s.fg_256 = 141;                   break;
        case TokenKind::String:       s.fg_256 = 150;                   break;
        case TokenKind::StringEscape: s.fg_256 = 215; s.bold = true;    break;
        case TokenKind::Operator:     s.fg_256 = 251;                   break;
        case TokenKind::Punctuation:  s.fg_256 = 247;                   break;
        case TokenKind::Tag:          s.fg_256 = 75;  s.bold = true;    break;
        case TokenKind::Attribute:    s.fg_256 = 117;                   break;
        case TokenKind::Property:     s.fg_256 = 117;                   break;
        case TokenKind::Section:      s.fg_256 = 39;  s.bold = true;    break;
        case TokenKind::Selector:     s.fg_256 = 220; s.bold = true;    break;
        case TokenKind::Decorator:    s.fg_256 = 220; s.italic = true;  break;
        case TokenKind::Preprocessor: s.fg_256 = 207;                   break;
        case TokenKind::Variable:     s.fg_256 = 215;                   break;
        case TokenKind::Heading:      s.fg_256 = 39;  s.bold = true;    break;
        case TokenKind::Emphasis:     s.italic = true;                  break;
        case TokenKind::Url:          s.fg_256 = 33;  s.underline = true; break;
        case TokenKind::Error:        s.fg_256 = 196; s.bold = true;    break;
    }
    return s;
}
...
void highlight_to_stream(std::string_view source,
                         Language lang,
                         ColorMode color,
    ...
    auto sink = [&](TokenKind kind, std::string_view text) {
        ...
        for (size_t i = 0; i < text.size(); ++i) {
            if (text[i] == '\n') {
                if (i > start) {
                    if (kind != run_kind) { flush_run(); run_kind = kind; }
